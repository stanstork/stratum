    ...
    ($join_clause:expr) => {
        (|| -> Result<query_builder::ast::expr::Expr, $crate::error::DbError> {
            ...
            if conditions.is_empty() {
                return Err($crate::error::DbError::QueryBuildError(
                    "JoinClause must have at least one condition.".to_string(),
                ));
            }

            let condition_to_expr = |cond: &$crate::sql::join::clause::JoinCondition| {
...
macro_rules! sql_filter_expr {
    ($filter_expr:expr) => {{
        fn convert(
            expr: &$crate::sql::filter::expr::SqlFilterExpr,
        ) -> Result<query_builder::ast::expr::Expr, $crate::error::DbError> {
            match expr {
                $crate::sql::filter::expr::SqlFilterExpr::Leaf(cond) => {
                    let op = match cond.comparator.as_str() {
                        ...
                        other => Err($crate::error::DbError::QueryBuildError(format!(
                ...
                $crate::sql::filter::expr::SqlFilterExpr::And(children)
                | $crate::sql::filter::expr::SqlFilterExpr::Or(children) => {
                    ...
                    let op = if matches!(expr, $crate::sql::filter::expr::SqlFilterExpr::And(_)) {
    ...
    ($builder:expr, $joins:expr) => {{
        $joins.iter().fold($builder, |b, join| {
            let join_kind = match join.join_type {
                $crate::sql::join::clause::JoinType::Inner => {
                    query_builder::ast::common::JoinKind::Inner
                }
                $crate::sql::join::clause::JoinType::Left => {
                    query_builder::ast::common::JoinKind::Left
                }
                $crate::sql::join::clause::JoinType::Right => {
                    query_builder::ast::common::JoinKind::Right
                }
                $crate::sql::join::clause::JoinType::Full => {
