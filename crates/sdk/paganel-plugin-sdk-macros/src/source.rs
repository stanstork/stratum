...
/// Expand `#[paganel_source(...)]` into the full plugin module.
pub fn expand(attr: AttrArgs, user_fn: ItemFn) -> syn::Result<TokenStream> {
    ...
    if attr.output.is_some() {
        return Err(syn::Error::new(
            span,
            "`output` is not valid on #[paganel_source]; use `output_schema = [...]` instead",
        ));
    }
    ...
    // it via `paganel_plugin_sdk::source_config()`.
    ...
    let role_entry = quote! {
        ...
        pub extern "C" fn __paganel_read_page(ptr: u32, len: u32) -> u64 {
            ...
            let result = ::std::panic::catch_unwind(
                || -> ::std::result::Result<::std::vec::Vec<u8>, ::paganel_plugin_sdk::PluginError> {
                    let cursor = ::paganel_plugin_sdk::runtime::parse_cursor(&input_bytes)?;
                    let page: ::paganel_plugin_sdk::SourcePage = #user_ident(cursor)?;
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn __paganel_estimated_count() -> i64 {
            -1
        }
    };
