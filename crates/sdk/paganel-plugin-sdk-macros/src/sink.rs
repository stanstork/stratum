...
/// Expand `#[paganel_sink(...)]` into the full plugin module.
pub fn expand(attr: AttrArgs, user_fn: ItemFn) -> syn::Result<TokenStream> {
    ...
    if attr.output.is_some() {
        return Err(syn::Error::new(
            span,
            "`output` is not valid on #[paganel_sink]; sinks return WriteResult",
        ));
    }
        ...
        Some(name) => {
            ...
            quote! {
                let result = ::std::panic::catch_unwind(
                    || -> ::paganel_plugin_sdk::PluginResult<()> { #id() },
                );
                match result {
    ...
    let role_entry = quote! {
        #[unsafe(no_mangle)]
        pub extern "C" fn __paganel_prepare(_ptr: u32, _len: u32) -> u32 {
            #prepare_body
        }

        #[unsafe(no_mangle)]
        pub extern "C" fn __paganel_finalize() -> u32 {
            #finalize_body
        }
        ...
        pub extern "C" fn __paganel_write_batch(ptr: u32, len: u32) -> u64 {
            ...
            let result = ::std::panic::catch_unwind(
                || -> ::std::result::Result<::std::vec::Vec<u8>, ::paganel_plugin_sdk::PluginError> {
                    let batch = ::paganel_plugin_sdk::PluginBatch::from_json_bytes(&input_bytes)?;
                    let res: ::paganel_plugin_sdk::WriteResult = #user_ident(batch)?;
