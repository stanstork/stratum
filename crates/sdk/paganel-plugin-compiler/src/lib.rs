...
static DEFAULT_RUNTIME_WASM: &[u8] = include_bytes!("../assets/paganel-plugin-js-runtime.wasm");

/// The `@paganel/plugin-sdk` JavaScript source, embedded so
/// `require("@paganel/plugin-sdk")` resolves with no `node_modules` on disk.
...
const SDK_FILES: &[(&str, &str)] = &[
    (
        "index.js",
        include_str!("../../paganel-plugin-sdk-js/src/index.js"),
    ),
    (
        "registry.js",
        include_str!("../../paganel-plugin-sdk-js/src/registry.js"),
    ),
    (
        "http.js",
        include_str!("../../paganel-plugin-sdk-js/src/http.js"),
    ),
    (
        "log.js",
        include_str!("../../paganel-plugin-sdk-js/src/log.js"),
    ),
    (
        "kv.js",
        include_str!("../../paganel-plugin-sdk-js/src/kv.js"),
    ),
    (
        "metrics.js",
        include_str!("../../paganel-plugin-sdk-js/src/metrics.js"),
    ),
    (
        "env.js",
        include_str!("../../paganel-plugin-sdk-js/src/env.js"),
    ),
    (
        "fs.js",
        include_str!("../../paganel-plugin-sdk-js/src/fs.js"),
    ),
];

/// Size of the `USER_JS` placeholder baked into the runtime WASM.
/// Must match `paganel-plugin-js-runtime/src/metadata.rs`.
...
/// `paganel-plugin-js-runtime/src/metadata.rs`.
const USER_JS_MAGIC: &[u8; 16] = b"PAGANEL_USR_JS01";
const METADATA_MAGIC: &[u8; 16] = b"PAGANEL_META_J01";
...
pub struct CompileOpts {
    ...
    /// `$PAGANEL_JS_RUNTIME`, then `runtime_wasm_bytes`, then the runtime
...
/// `$PAGANEL_ESBUILD`, then `esbuild` on `PATH`, then `npx esbuild` when Node is
...
fn resolve_esbuild(opts: &CompileOpts) -> Result<Bundler, CompileError> {
    ...
    if let Some(p) = std::env::var_os("PAGANEL_ESBUILD") {
    ...
    Err(CompileError::Bundle(
        "no esbuild found: install esbuild (or Node.js for `npx`), set $PAGANEL_ESBUILD, \
...
fn materialize_sdk() -> Result<PathBuf, CompileError> {
    ...
    let key = hasher.finalize().to_hex();
    let dir = std::env::temp_dir().join(format!("paganel-plugin-sdk-{}", &key.as_str()[..16]));
...
/// Bundle the user's JS together with `@paganel/plugin-sdk` into a single
...
fn bundle_with_esbuild(input: &Path, opts: &CompileOpts) -> Result<String, CompileError> {
    ...
    // Resolve `@paganel/plugin-sdk` to the embedded copy regardless of the
    ...
    // The SDK's registry module attaches __paganel_get_metadata /
    // __paganel_dispatch to globalThis as a load-time side effect, so the
...
fn extract_metadata(bundle: &str) -> Result<serde_json::Value, CompileError> {
    ...
    let json: String = context.with(|ctx| -> Result<String, CompileError> {
        ...
        let get_md: Function = ctx.globals().get("__paganel_get_metadata").map_err(|_| {
    ...
    let (id, off) = located.ok_or_else(|| {
        CompileError::Patch(format!(
            "could not find the {label} placeholder marker in the runtime WASM; \
             is the paganel-plugin-js-runtime.wasm blob up to date?"
        ))
    })?;
...
fn validate_exports(wasm: &[u8], role: &str) -> Result<(), CompileError> {
    ...
    let role_entry = match role {
        "transform" => "__paganel_transform",
        "filter" => "__paganel_evaluate",
        "source" => "__paganel_read_page",
        "sink" => "__paganel_write_batch",
        other => {
...
/// `runtime_wasm` path, then `$PAGANEL_JS_RUNTIME`, then a caller-supplied
...
fn load_runtime_wasm(opts: &CompileOpts) -> Result<Vec<u8>, CompileError> {
    ...
    if let Some(path) = opts
        .runtime_wasm
        .clone()
        .or_else(|| std::env::var_os("PAGANEL_JS_RUNTIME").map(PathBuf::from))
    {
