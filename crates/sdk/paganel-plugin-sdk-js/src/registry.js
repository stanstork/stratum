...
const STORE = {
    ...
    config: {},            // plugin config (PPL `config { }`), set by the host at init
};

// Install the plugin's static config.
function __paganel_set_config(configJson) {
...
// `prepare`/`finalize`; the runtime invokes them via __paganel_lifecycle.
...
function __paganel_get_metadata() {
...
function __paganel_dispatch(role, inputJson) {
    if (STORE.role !== role) {
...
function __paganel_lifecycle(hook, inputJson) {
    const fn =
...
module.exports = {
    ...
    setSinkPrepare,
    setSinkFinalize,
    __paganel_get_metadata,
    __paganel_dispatch,
    __paganel_lifecycle,
    __paganel_set_config,
};

// Expose the hooks on globalThis as a load-time side effect. The Paganel JS
// runtime evaluates the bundle and then reads these off globalThis (see
// paganel-plugin-js-runtime/src/bootstrap.rs); the compiler's QuickJS-based
...
if (typeof globalThis !== "undefined") {
    globalThis.__paganel_get_metadata = __paganel_get_metadata;
    globalThis.__paganel_dispatch = __paganel_dispatch;
    globalThis.__paganel_lifecycle = __paganel_lifecycle;
    globalThis.__paganel_set_config = __paganel_set_config;
}
