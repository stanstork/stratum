...
pub fn initialize(cfg_ptr: u32, cfg_len: u32) -> Result<JsRuntime, String> {
    ...
    // After eval, the SDK has populated globalThis with __paganel_dispatch
    // and __paganel_get_metadata. Grab persistent handles, and hand the plugin
    // its config (the PPL `config { }` block) via __paganel_set_config.
    let (dispatch_fn, metadata_json) = context.with(|ctx| -> Result<_, String> {
        let global = ctx.globals();

        if let Ok(set_cfg) = global.get::<_, Function>("__paganel_set_config") {
...
/// `__paganel_dealloc` using the same size.
#[doc(hidden)]
