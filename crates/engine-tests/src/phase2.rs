...
mod tests {
    ...
    //
    // Config: examples/configs/phase2/p2-01-schema-only.smql
    //
    // Scenario:
    ...
    async fn schema_only_creates_all_sakila_tables_empty() {
        ...
        // CARGO_MANIFEST_DIR = crates/engine-tests; workspace root is two levels up
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-01-schema-only.smql"
        );
    ...
    //
    // Config: examples/configs/phase2/p2-02-cascade-data.smql
    //
    // Scenario:
    ...
    async fn cascade_migrates_schema_and_data_with_fk_integrity() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-02-cascade-data.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-04-depth-limit.smql
    //
    // Scenario:
    ...
    async fn depth_limit_creates_only_direct_fk_tables() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-04-depth-limit.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-08-enum-migration.smql
    //
    // Scenario:
    ...
    async fn enum_type_created_before_table_with_correct_labels() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-08-enum-migration.smql"
        );
    ...
    //
    // Config: examples/configs/phase2/p2-07-schema-dedup.smql
    //
    // Scenario:
    ...
    async fn schema_dedup_across_pipelines_no_duplicate_errors() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-07-schema-dedup.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-06-circular-fk.smql
    //
    // Scenario:
    ...
    async fn circular_fk_migrates_with_both_constraints_intact() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-06-circular-fk.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-09-generated-columns.smql
    //
    // Scenario:
    ...
    async fn generated_columns_migrated_with_correct_expressions_and_data() {
        ...
        let result = async {
            let config = concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/../../examples/configs/phase2/p2-09-generated-columns.smql"
            );
            run_smql_file(config)
                .await
    ...
    //
    // Config: examples/configs/phase2/p2-05-exclusion.smql
    //
    // Scenario:
    ...
    async fn exclusion_creates_only_non_excluded_tables() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-05-exclusion.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-03-full-chain.smql
    //
    // Scenario:
    ...
    async fn full_chain_fk_constraints_satisfied() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-03-full-chain.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-10-table-rename.smql
    //
    // Scenario:
    ...
    async fn table_rename_map_creates_renamed_tables_with_fk_integrity() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-10-table-rename.smql"
        );
        run_smql_file(config)
            .await
    ...
    //
    // Config: examples/configs/phase2/p2-11-full-sakila.smql
    //
    // Scenario:
    ...
    async fn full_sakila_warehouse_naming_with_computed_column() {
        ...
        let config = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../examples/configs/phase2/p2-11-full-sakila.smql"
        );
        run_smql_file(config)
            .await
