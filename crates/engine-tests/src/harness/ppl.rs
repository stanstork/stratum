...
fn strip_preamble(ppl: &str) -> String {
    let mut out = Vec::new();
    let mut depth = 0usize;

    for line in ppl.lines() {
        if depth == 0 {
    ...
    fn configs_declare_no_connections() {
        ...
        for group in std::fs::read_dir(root).expect("configs/") {
            ...
            for file in std::fs::read_dir(&group).expect("config group") {
                ...
                if path.extension().and_then(|e| e.to_str()) != Some("ppl") {
                    continue;
                }
