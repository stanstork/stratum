use crate::io::format::DataFormat;
use connectors::{
    error::DriverError,
    sql::join::{
...
impl LinkedSource {
    /// Create a new linked source from a driver that implements schema introspection.
    pub async fn new<D>(
        driver: Arc<D>,
        format: &DataFormat,
        joins: &[Join],
        ...
        D: SchemaIntrospector,
    {
        ...
        if !matches!(format, DataFormat::MySql | DataFormat::Postgres) {
            return Err(DriverError::UnsupportedFormat(format.to_string()));
        }

        let tables = joins.iter().map(|j| j.table.clone()).collect::<Vec<_>>();

        // Build a map from table name to alias for projection extraction
        let table_to_alias: HashMap<String, String> = joins
            .iter()
            .map(|j| (j.table.clone(), j.alias.clone()))
            .collect();

        // precompute join clauses & projection
        let join_clauses = build_join_clauses(joins);
        let projection = extract_projection(&tables, &table_to_alias, mapping);

        // fetch metadata for all tables
        let mut meta = HashMap::new();
        for table in &tables {
            let table_meta = driver.table_metadata(table).await?;
...
pub fn build_join_clauses(joins: &[Join]) -> Vec<JoinClause> {
    joins
        .iter()
        .filter_map(|join| {
            ...
            if let CompiledExpression::Binary { left, right, .. } = condition {
                ...
                let (left_table_alias, left_column) = extract_dotpath_parts(left)?;
                let (right_table_alias, right_column) = extract_dotpath_parts(right)?;
...
pub fn extract_projection(
    ...
    tables
        .iter()
        .map(|table| {
            // Get the alias for this table (cross-entity refs are keyed by alias)
            let alias = table_to_alias.get(table).unwrap_or(table);

            // Get all cross-entity references for this table's alias
            let refs = metadata.get_cross_entity_refs_for(alias);
