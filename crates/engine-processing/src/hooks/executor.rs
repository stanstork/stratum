...
use connectors::{
    adapter::Adapter,
    sql::base::{adapter::SqlAdapter, transaction::Transaction},
};
...
impl HookExecutor {
    pub fn new(adapter: Arc<Adapter>, hooks: LifecycleHooks) -> Self {
        Self {
            adapter,
            hooks,
    ...
    async fn execute_sql(
        ...
        match self.adapter.as_ref() {
            Adapter::MySql(_adapter) => {
                unimplemented!("MySQL adapter hook execution not implemented yet");
            }
            Adapter::Postgres(adapter) => {
                // Start a new transaction for this hook
                let mut client = adapter.lock_client().await;
                let tx = Transaction::PgTransaction(client.transaction().await.map_err(|e| {
                ...
                match adapter.exec_tx(&tx, sql).await {
                    Ok(_) => {
            ...
            _ => {
                ...
                Err(HookError::ExecutionFailed {
                    ...
                    source: Box::new(std::io::Error::other(
                        "Unsupported adapter for hook execution",
                    )),
                })
            }
        }
