...
pub fn eval_date(args: &[Cow<'_, Value>], _ctx: &EvalContext) -> Result<Value> {
    match args.first().map(|c| &**c) {
...
pub fn eval_year(args: &[Cow<'_, Value>], _ctx: &EvalContext) -> Result<Value> {
    extract("year", args, |d| d.year() as i64)
}

/// `month(ts)` - extract the month (1-12) as an integer.
pub fn eval_month(args: &[Cow<'_, Value>], _ctx: &EvalContext) -> Result<Value> {
    extract("month", args, |d| d.month() as i64)
}

/// `quarter(ts)` - extract the quarter (1-4) as an integer.
pub fn eval_quarter(args: &[Cow<'_, Value>], _ctx: &EvalContext) -> Result<Value> {
    extract("quarter", args, |d| ((d.month0() / 3) + 1) as i64)
}

/// `now()` - the current UTC timestamp.
pub fn eval_now(_args: &[Cow<'_, Value>], _ctx: &EvalContext) -> Result<Value> {
...
fn extract(function: &str, args: &[Cow<'_, Value>], f: impl Fn(NaiveDate) -> i64) -> Result<Value> {
    match args.first().map(|c| &**c) {
        Some(Value::Null) | None => Ok(Value::Null),
        Some(v) => as_date(v)
            .map(|d| Value::Int(f(d)))
...
mod tests {
    ...
    fn a<const N: usize>(vs: [Value; N]) -> [Cow<'static, Value>; N] {
        vs.map(Cow::Owned)
    }
    ...
    fn test_date_from_timestamp() {
        with_dummy_ctx(|ctx| {
            let result = eval_date(&a([ts(2024, 7, 15)]), ctx).unwrap();
            assert_eq!(
                result,
    ...
    fn test_year_month_quarter() {
        with_dummy_ctx(|ctx| {
            assert_eq!(
                eval_year(&a([ts(2024, 11, 2)]), ctx).unwrap(),
                Value::Int(2024)
            );
            assert_eq!(
                eval_month(&a([ts(2024, 11, 2)]), ctx).unwrap(),
                Value::Int(11)
            );
            // November is in Q4.
            assert_eq!(
                eval_quarter(&a([ts(2024, 11, 2)]), ctx).unwrap(),
                Value::Int(4)
            );
            // January is in Q1.
            assert_eq!(
                eval_quarter(&a([ts(2024, 1, 2)]), ctx).unwrap(),
                Value::Int(1)
            );
        });
    }
    ...
    fn test_null_propagates() {
        with_dummy_ctx(|ctx| {
            assert_eq!(eval_year(&a([Value::Null]), ctx).unwrap(), Value::Null);
    ...
    fn test_non_temporal_errors() {
        with_dummy_ctx(|ctx| {
            assert!(eval_year(&a([Value::Int(5)]), ctx).is_err());
        });
    }
